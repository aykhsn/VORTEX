...
/**********************
 liquid
***********************/
const createHtmlLiquid = () =>
	src( '*.html' )
		.pipe( replace(/src="dist\/assets\/(.*\..*)"/g, 'src="{{ \'$1\' | asset_url }}"') )
		.pipe( dest('dist/liquid') );

const createStyleLiquid = () =>
	src( 'dist/css/*.css' )
		.pipe( replace(/url\(\"\.\.\/assets\/(.*\.\w*)\"\)/g, 'url\("{{ \'$1\' | asset_url }}"\)' ) )
		.pipe( dest('dist/liquid') );
...
exports.liquid = parallel( createHtmlLiquid, createStyleLiquid );
